...
static inline void errorPrint(std::string str) {
  std::cerr << "[WirelessFT] " << str << std::endl;
}

WirelessFT::WirelessFT(bool verbose)
    : mIsStreaming(false), mVerbose(verbose), mTelnetSocket(-1),
      mUDPSocket(-1) {
...
bool WirelessFT::telnetConnect(std::string hostname, int port) {
  ...
  if (connect(mTelnetSocket, (sockaddr *)&serv_addr, sizeof(serv_addr)) < 0) {
    errorPrint(
        string_format("error connecting to socket at host '%s' port '%d'",
                      hostname.c_str(), port));
    close(mTelnetSocket);
    mTelnetSocket = -1;
    return false;
  ...
  int result = setsockopt(mTelnetSocket, IPPROTO_TCP, TCP_NODELAY,
...
bool WirelessFT::telnetCommand(std::string &response, std::string command,
                               unsigned int micros) {
  if (mTelnetSocket < 0) {
    response = "";
    return false;
  ...
  try {
    verbosePrint(mVerbose,
                 string_format("sending telnet command '%s'", command.c_str()));
    ...
    if (n < 0) {
      ...
      verbosePrint(mVerbose,
                   string_format("socket write: sent %d bytes, ok.", n));
    }
    ...
    if (n < 0) {
      ...
      verbosePrint(mVerbose,
                   string_format("telnet socket read: got %d bytes.", n));
      response = std::string(buffer);
    }

    verbosePrint(mVerbose,
                 string_format("telnet response: '%s'", response.c_str()));
  } catch (...) {
    response = "";
    return false;
  }
...
toSystemTimestamp(unsigned long timestamp) {
  ...
  std::chrono::time_point<std::chrono::system_clock, std::chrono::microseconds>
      deviceBase = std::chrono::floor<std::chrono::seconds>(
          std::chrono::system_clock::now());
  uint64_t modsecs = (std::chrono::duration_cast<std::chrono::seconds>(
                          deviceBase.time_since_epoch())
                          .count() +
                      NTP_TO_UNIX) %
                     (1 << 20);
